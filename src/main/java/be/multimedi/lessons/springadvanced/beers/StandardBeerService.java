...
public class StandardBeerService implements BeerService {
    ...
    public int orderBeer(String name, int beerId, int amount) throws InvalidAmountException, InvalidBeerException {
        List<BeerOrderItem> items = createOrderItems(new int[] {beerId}, new int[] {amount});
    ...
    List<BeerOrderItem> createOrderItems(int[] beerIds, int[] amounts) throws InvalidBeerException, InvalidAmountException {
    ...
    private Beer getBeer(int id) throws InvalidBeerException {
        Optional<Beer> beerOptional = Optional.ofNullable(beerRepository.getBeerById(id));
        Beer beer = beerOptional.orElseThrow(() -> new InvalidBeerException(String.format("Beer with ID '%d' doesn't exist.", id)));
        return beer;
    }
